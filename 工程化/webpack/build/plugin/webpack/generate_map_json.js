...
let tool = {
  create_path_sync: function (dir_path, mode) {
    ...
    let path_uri;
    // 将路径按分隔符拆分
    dir_path.split(path.sep)
      .forEach(function (sub_dir) {
    ...
    this.plugin('done', function (stats) {
      ...
      assets.forEach(function (item) {
        ...
        if(!filename){
          ...
          let start = name.indexOf(static_root)
          start += static_root.length + 1
          let end = name.length
          const arr = name.slice(start, end).split('.')
      ...
      let webpackOutputFile = path.join(json.outputPath, 'map.json');
