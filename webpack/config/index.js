...
module.exports = {
  build: {
    ...
    zipAll:path.resolve(__dirname,'../dist/', 'html', 'dist.zip'),
    zip:path.resolve(__dirname,'../dist/', 'html', 'vue.zip'),
    index: path.resolve(__dirname, project_config.publishWebServiceResourcePath + 'vue/index.html'),
    htmlRoot: path.resolve(__dirname, '../dist/', 'html', project_config.publishWebServiceResourcePath), //html文件根路径
    ...
    dllEntry:{
      vendors: ['vuex','axios','vue-router','vue-navigation','babel-polyfill'],
      // uiLibs: ['hui-mobile','element-ui'], 此处打进去element-ui，页面上渲染不出el-table组件
  ...
  dev: {
    ...
    assetsPublicPath: '/',
    proxyTable: {
      '/dajia-fe-scaffolding/api': {
        target: 'https://yapi.g2work.com/mock/33',
        // target: 'https://yapi.g2work.com/mock/24',
        changeOrigin: true,
        pathRewrite: {
          '^/dajia-fe-scaffolding/api': ''
        }
    ...
    cssSourceMap: false,
    isServerMultiplePages: true, // 是否开启多页服务
  },
  debug: {
    // 服务器端配置
    env: require('./debug.env'),
    prodEnv: require('./debug.env'),
    index: path.resolve(__dirname, project_config.debugWebServiceResourcePath + 'vue/index.html'),
